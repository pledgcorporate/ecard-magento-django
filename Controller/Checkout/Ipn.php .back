...
class Ipn extends Action
{
    ...
    private function handleBackMode(array $params): void
    {
        ...
        if (in_array($pledgStatus, self::STATUS_COMPLETED)) {
            ...
            $this->invoiceOrder($order, $transactionId, __(
                'Received invoicing order from Django back notification with status %1',
                $pledgStatus
            ));
        ...
        if (in_array($pledgStatus, self::STATUS_CANCELLED)) {
            ...
            $order->registerCancellation(__(
                'Received cancellation order from Django back notification with status %1',
                $pledgStatus
            ))->save();
        ...
        if (in_array($pledgStatus, self::STATUS_PENDING)) {
            ...
            $this->addMessageOnOrder($order, __(
                'Received Django back notification with status %1. Waiting for further instructions to update order.',
                $pledgStatus
            ));
        ...
        $this->logger->error('Received unhandled status from Django back notification', ['status' => $pledgStatus]);
    }
    ...
    private function handleTransferMode(array $params): void
    {
        ...
        $this->invoiceOrder($order, $transactionId, __('Received invoicing order from Django transfer notification'));
    }
    ...
    private function getOrder(array $params): Order
    {
        ...
        if (!in_array($paymentMethod, ConfigProvider::getPaymentMethodCodes())) {
            throw new \Exception(sprintf('Order with method %s should not be updated via Django notification', $paymentMethod));
        }
